...
for label = 1:num_labels
  ...
  cost = ((1/m) * (-y_binarized'*log(sigmoid(X * theta))- (1-y_binarized)'*log(1-sigmoid((X*theta)))));
  regularization = (lambda/(2*m))*sum(theta(2:end,:).^2);
  J = cost + regularization;

  theta_for_grad = [0; theta(2:end,:)];
  grad =  (lambda/m)*theta_for_grad + (1/m) * X' * (sigmoid(X*theta)-y);

  all_theta(label,:) = (theta - grad)';
end
